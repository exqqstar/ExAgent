...
impl IndexDb {
    pub async fn upsert_project(&self, input: ProjectUpsert) -> anyhow::Result<ProjectRecord> {
        ...
        sqlx::query(
            r#"
INSERT INTO projects (id, name, path, archived_at, pinned, created_at, last_opened_at)
VALUES (?, ?, ?, NULL, 0, ?, ?)
ON CONFLICT(path) DO UPDATE SET
  ...
  last_opened_at = excluded.last_opened_at
    ...
    pub async fn project_by_id(&self, project_id: &str) -> anyhow::Result<ProjectRecord> {
        let row =
            sqlx::query("SELECT id, name, path, archived_at, pinned FROM projects WHERE id = ?")
                .bind(project_id)
                .fetch_one(self.pool())
                .await?;
    ...
    pub async fn list_projects(&self) -> anyhow::Result<Vec<ProjectRecord>> {
        let rows = sqlx::query(
            "SELECT id, name, path, archived_at, pinned FROM projects WHERE archived_at IS NULL ORDER BY pinned DESC, last_opened_at DESC, name ASC",
        )
        .fetch_all(self.pool())
        .await?;
        rows.into_iter()
    ...
    pub async fn set_project_pinned(&self, project_id: &str, pinned: bool) -> anyhow::Result<()> {
    ...
    pub async fn archive_project(&self, project_id: &str) -> anyhow::Result<()> {
    ...
    pub async fn archive_project_threads(&self, project_id: &str) -> anyhow::Result<()> {
        sqlx::query(
            "UPDATE threads SET archived_at = ? WHERE project_id = ? AND archived_at IS NULL",
        )
        .bind(time::now_unix_millis())
        .bind(project_id)
        .execute(self.pool())
...
fn project_record_from_row(row: &sqlx::sqlite::SqliteRow) -> anyhow::Result<ProjectRecord> {
    Ok(ProjectRecord {
        id: row.try_get("id")?,
...
mod tests {
    ...
    async fn insert_thread(db: &IndexDb, project_id: &str, thread_id: &str) {
        sqlx::query(
            r#"
INSERT INTO threads (
...
VALUES (?, ?, ?, ?, ?, 'test', 0, 'idle', 1, 1)
        ...
        .bind(format!("{thread_id} title"))
        .bind(format!("{thread_id} preview"))
        .execute(db.pool())
        .await
        .unwrap();
    }
    ...
    #[tokio::test]
    async fn project_metadata_actions_update_visible_project_list() {
        let (dir, db) = temp_db().await;
        let alpha = add_project(&db, &dir.path().join("alpha"), "Alpha").await;
        let beta = add_project(&db, &dir.path().join("beta"), "Beta").await;

        db.rename_project(&alpha.id, "Renamed Alpha").await.unwrap();
        db.set_project_pinned(&alpha.id, true).await.unwrap();
        db.archive_project(&beta.id).await.unwrap();

        let projects = db.list_projects().await.unwrap();
        assert_eq!(projects.len(), 1);
        assert_eq!(projects[0].id, alpha.id);
        assert_eq!(projects[0].name, "Renamed Alpha");
        assert!(projects[0].pinned);
        assert!(projects[0].archived_at.is_none());
    }

    #[tokio::test]
    async fn removing_project_deletes_its_indexed_threads() {
        let (dir, db) = temp_db().await;
        let project = add_project(&db, &dir.path().join("alpha"), "Alpha").await;
        insert_thread(&db, &project.id, "thread-a").await;

        db.remove_project(&project.id).await.unwrap();
    ...
    #[tokio::test]
    async fn archiving_project_conversations_soft_archives_all_threads() {
        let (dir, db) = temp_db().await;
        let project = add_project(&db, &dir.path().join("alpha"), "Alpha").await;
        insert_thread(&db, &project.id, "thread-a").await;
        insert_thread(&db, &project.id, "thread-b").await;

        db.archive_project_threads(&project.id).await.unwrap();
        ...
        assert!(archived.iter().all(|thread| thread.archived_at.is_some()));
    }
}
