...
pub fn provider_env_api_key(provider_id: &str) -> Option<String> {
    provider_env_vars(provider_id, ProviderEnvKind::ApiKey)
        .into_iter()
...
pub fn provider_env_base_url(provider_id: &str) -> Option<String> {
    provider_env_vars(provider_id, ProviderEnvKind::BaseUrl)
        .into_iter()
        .find_map(|key| std::env::var(key).ok())
        .filter(|value| !value.trim().is_empty())
}
...
fn provider_env_vars(provider_id: &str, kind: ProviderEnvKind) -> Vec<&'static str> {
    match (provider_id, kind) {
        ("openai", ProviderEnvKind::ApiKey) => vec!["OPENAI_API_KEY"],
        ("openai", ProviderEnvKind::BaseUrl) => vec!["OPENAI_BASE_URL"],
        ("anthropic", ProviderEnvKind::ApiKey) => vec!["ANTHROPIC_API_KEY"],
        ("anthropic", ProviderEnvKind::BaseUrl) => vec!["ANTHROPIC_BASE_URL"],
        ("google", ProviderEnvKind::ApiKey) => vec!["GOOGLE_API_KEY", "GEMINI_API_KEY"],
        ("google", ProviderEnvKind::BaseUrl) => vec!["GOOGLE_BASE_URL", "GEMINI_BASE_URL"],
        ("deepseek", ProviderEnvKind::ApiKey) => vec!["DEEPSEEK_API_KEY"],
        ("deepseek", ProviderEnvKind::BaseUrl) => vec!["DEEPSEEK_BASE_URL"],
        ("kimi", ProviderEnvKind::ApiKey) => vec!["KIMI_API_KEY", "MOONSHOT_API_KEY"],
        ("kimi", ProviderEnvKind::BaseUrl) => vec!["KIMI_BASE_URL", "MOONSHOT_BASE_URL"],
        ("glm", ProviderEnvKind::ApiKey) => {
            vec!["GLM_API_KEY", "ZHIPU_API_KEY", "BIGMODEL_API_KEY"]
        }
        ("glm", ProviderEnvKind::BaseUrl) => {
            vec!["GLM_BASE_URL", "ZHIPU_BASE_URL", "BIGMODEL_BASE_URL"]
        }
        _ => Vec::new(),
    }
}
